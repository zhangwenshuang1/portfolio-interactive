...
// 6 块拼图初始槽位：分发在板面(1680×1120)外圈四处——四角 + 顶中 + 底中，
// 彼此远离、中间留出大片空白，让"读完后六块飞向中央聚拢"的动作清晰可见。
const SLOT_POSITIONS = [
  { x: 40, y: 60 }, //   photo（左上）
  { x: 620, y: 40 }, //  ent（顶中）
  { x: 1200, y: 60 }, //  doc（右上）
  { x: 40, y: 640 }, //  brand（左下）
  { x: 620, y: 620 }, // ai_comic（中下）
  { x: 1200, y: 640 }, // sports（右下）
]

// 读完所有故事后：六块拼图飞向中央聚拢成的"完整长方形"紧密布局。
// 块元素 440，水平/垂直相邻间距均为 440（元素边界相切）→ 凸起凹槽正好互相咬合，
// 合成一块完整的大拼图（3 列 × 2 行，宽 1320 × 高 880），整体居中于板面(840,560)。
export const ASSEMBLY_POSITIONS = [
  { x: 180, y: 120 }, //  photo（左上）
  { x: 620, y: 120 }, //  ent（中上）
  { x: 1060, y: 120 }, // doc（右上）
  { x: 180, y: 560 }, //  brand（左下）
  { x: 620, y: 560 }, //  ai_comic（中下）
  { x: 1060, y: 560 }, // sports（右下）
]
