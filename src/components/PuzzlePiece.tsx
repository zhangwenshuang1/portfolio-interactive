...
// 每个内部接缝都做成"经典锯齿"：一条边上有一个凸起(圆帽)、一个凹槽(圆坑)，
// 相邻块在相同位置一一互补（这一块凸，邻块就凹）→ 波浪咬合、严丝合缝。
// 每个圆帽半径 R=40，圆心落在共享中缝（即该边边界 0 或 440），
// 一条边两个圆心在 t=140 与 t=300，正好交错补平。外缘(无邻居的边)全部平直，
// 保证外围拼成规则的完整长方形。
// 每条缝"本边侧极性"(凸=1/凹=0)：凸=「A r r 0 0 1」凹=sweep0。
const getPieceShape = (id: string) => {
  const shapes: Record<string, string> = {
    // 左上 photo：右凸/右凹(咬ent)、底凹/底凸(咬brand)（竖缝上凸下凹、横缝左扁）
    photo:
      'path("M 0 0 L 440 0 L 440 100 A 40 40 0 0 1 440 180 L 440 260 A 40 40 0 0 0 440 340 L 440 440 L 340 440 A 40 40 0 0 0 260 440 L 180 440 A 40 40 0 0 1 100 440 L 0 440 L 0 0 Z")',
    // 中上 entertainment：顶平；右凹/右凸(咬doc)、底凹/底凸(咬ai)、左凹/左凸(配photo)
    entertainment:
      'path("M 0 0 L 440 0 L 440 100 A 40 40 0 0 0 440 180 L 440 260 A 40 40 0 0 1 440 340 L 440 440 L 340 440 A 40 40 0 0 1 260 440 L 180 440 A 40 40 0 0 0 100 440 L 0 440 L 0 340 A 40 40 0 0 1 0 260 L 0 180 A 40 40 0 0 0 0 100 L 0 0 Z")',
    // 右上 documentary：顶/右平；底凹/底凸(咬sports)、左凹/左凸(配ent)
    documentary:
      'path("M 0 0 L 440 0 L 440 440 L 340 440 A 40 40 0 0 0 260 440 L 180 440 A 40 40 0 0 1 100 440 L 0 440 L 0 340 A 40 40 0 0 0 0 260 L 0 180 A 40 40 0 0 1 0 100 L 0 0 Z")',
    // 左下 brand：左/下平；顶凸凹(配photo，本侧顶-凸凹)、右凸/右凹(咬ai)
    brand:
      'path("M 0 0 L 100 0 A 40 40 0 0 0 180 0 L 260 0 A 40 40 0 0 1 340 0 L 440 0 L 440 100 A 40 40 0 0 1 440 180 L 440 260 A 40 40 0 0 0 440 340 L 440 440 L 0 440 L 0 0 Z")',
    // 中下 ai_comic：底平；顶凸凹(配ent，本侧顶-凹凸)、右凹/右凸(咬sports)、左凹/左凸(配brand)
    ai_comic:
      'path("M 0 0 L 100 0 A 40 40 0 0 1 180 0 L 260 0 A 40 40 0 0 0 340 0 L 440 0 L 440 100 A 40 40 0 0 0 440 180 L 440 260 A 40 40 0 0 1 440 340 L 440 440 L 0 440 L 0 340 A 40 40 0 0 1 0 260 L 0 180 A 40 40 0 0 0 0 100 L 0 0 Z")',
    // 右下 sports：右/下平；顶凸凹(配doc，本侧顶-凹凸)、左凹/左凸(配ai)
    sports:
      'path("M 0 0 L 100 0 A 40 40 0 0 0 180 0 L 260 0 A 40 40 0 0 1 340 0 L 440 0 L 440 440 L 0 440 L 0 340 A 40 40 0 0 0 0 260 L 0 180 A 40 40 0 0 1 0 100 L 0 0 Z")',
  }
