...
// 每个内部接缝做成"凹凸互补"：一条边上一个圆帽(凸)或圆坑(凹)，
// 相邻块在相同位置一一互补（这一块凸，邻块就凹）→ 完美契合、严丝合缝。
// 每个圆帽半径 R=40，圆心落在共享中缝（即该边边界 0 或 440），
// 圆心在该边的中间（t≈180..260）。外缘(无邻居的边)全部平直，
// 保证外围拼成规则的完整长方形。
// 每条缝"本边侧极性"(凸=1/凹=0)：凸=「A r r 0 0 1」凹=sweep0。
const getPieceShape = (id: string) => {
  const shapes: Record<string, string> = {
    // 左上 photo：右凸(咬ent)、底凸(咬brand)，左/顶外平
    photo:
      'path("M 0 0 L 440 0 L 440 180 A 40 40 0 0 1 440 260 L 440 440 L 260 440 A 40 40 0 0 1 180 440 L 0 440 L 0 0 Z")',
    // 中上 entertainment：左凹(配photo)、右凹(咬doc)、底凹(咬ai)，顶外平
    entertainment:
    ...
    // 右下 sports：左凸(配ai)、顶凸(配doc)，右/底外平
    sports:
